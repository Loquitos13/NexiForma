...
export function FormadorDocumentosGestor({
  formadorId,
  documentos,
  requisicoes,
  canManage,
  ...
  const [partilharComFormador, setPartilharComFormador] = useState(false);
  ...
  const loadObrigatorios = useCallback(async () => {
    ...
    if (r.ok) setObrigatorios((await r.json()) as FormadorDocObrigatorioResumo);
  }, [formadorId]);
  ...
  async function onUpload(file: File) {
    ...
    const qs = new URLSearchParams({
      formadorId,
      categoria,
      visivelFormador: partilharComFormador ? "true" : "false",
    });
  ...
  async function criarRequisicao(e: FormEvent) {
    ...
    setReqTitulo("");
    setReqDescricao("");
    setMsg("Pedido de documento enviado ao formador.");
    await onRefresh();
  }
    ...
    <>
      ...
      <Card className="mb-6">
        <CardHeader className="border-b border-slate-700/40 flex flex-row flex-wrap items-center justify-between gap-3">
          ...
          {canManage ? (
            <div className="flex flex-wrap items-center gap-2">
              ...
              >
                {FORMADOR_DOC_CATEGORIAS_UPLOAD.map((c) => (
              ...
              <label className="inline-flex items-center gap-1.5 text-xs text-slate-300">
                <input
                  type="checkbox"
                  checked={partilharComFormador}
                  onChange={(e) => setPartilharComFormador(e.target.checked)}
                  className="rounded border-slate-600"
                />
                Partilhar com formador
              </label>
              <input
                ref={fileRef}
                type="file"
        ...
        <CardContent className="pt-4 space-y-3">
          ...
          {!partilharComFormador && canManage ? (
                ...
                >
                  <div className="min-w-0">
                    ...
                    <p className="text-[11px] text-slate-500">
                      ...
                      {d.visivelFormador === false ? " · interno" : ""}
                    </p>
                  </div>
                  <div className="flex items-center gap-2">
                    {d.visivelFormador === false ? (
