...
type AcaoOpt = { id: string; codigoInterno: string; titulo: string; dataInicio: string; dataFim: string };
type SessaoEvent = {
  id: string;
  numeroSessao: number;
  data: string;
...
export default function CalendarioPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const inicio = new Date(mesAtual.getFullYear(), mesAtual.getMonth() - 1, 1).toISOString().split("T")[0];
    const fim = new Date(mesAtual.getFullYear(), mesAtual.getMonth() + 2, 0).toISOString().split("T")[0];

    const evts: SessaoEvent[] = [];
    for (const acao of acoes) {
      const r = await bffFetch(`/api/v1/cronogramas?acaoFormacaoId=${encodeURIComponent(acao.id)}`, { headers: { accept: "application/json" } });
      if (!r.ok) continue;
      const cronos = (await r.json()) as { id: string }[];
      for (const c of cronos) {
        const sRes = await bffFetch(`/api/v1/sessoes-formacao?cronogramaId=${encodeURIComponent(c.id)}`, { headers: { accept: "application/json" } });
        if (!sRes.ok) continue;
        const sessoes = (await sRes.json()) as SessaoEvent[];
        for (const s of sessoes) {
          if (s.data >= inicio && s.data <= fim) {
            evts.push({ ...s, acaoCodigo: acao.codigoInterno, acaoTitulo: acao.titulo });
          }
        }
      }
    }
  ...
  useEffect(() => { if (acoes.length) void load(); }, [load]);
  ...
  for (let d = 1; d <= diasNoMes; d++) {
    dias.push(new Date(ano, mes, d).toISOString().split("T")[0]);
  }
    ...
    <div className="max-w-5xl space-y-5">
      ...
      <div className="rounded-2xl bg-slate-900/50 border border-slate-700/30 overflow-hidden">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-700/30">
          <button onClick={() => setMesAtual(new Date(ano, mes - 1))}
        ...
        <div className="grid grid-cols-7 text-center">
          {["Seg", "Ter", "Qua", "Qui", "Sex", "Sab", "Dom"].map((d) => (
            <div key={d} className="py-2 text-[11px] font-semibold text-slate-500 uppercase tracking-wider border-b border-slate-700/20">{d}</div>
          ))}
          {dias.map((data, i) => (
            <button
              key={i}
              ...
              className={`min-h-[72px] p-1.5 border border-slate-700/10 text-left transition-colors ${
                !data ? "bg-slate-900/30" :
                data === hoje ? "bg-blue-500/5" :
                data === selectedDate ? "bg-blue-500/10 ring-1 ring-blue-500/30" :
                ...
                <>
                  ...
                  {temEventos(data) ? (
                    <div className="mt-1 space-y-0.5">
                      {eventosDoDia(data).slice(0, 3).map((e) => (
                        <div key={e.id} className="text-[9px] leading-tight truncate px-1 py-0.5 rounded bg-blue-500/15 text-blue-300">
      ...
      {selectedDate ? (
        <div className="rounded-2xl bg-slate-900/50 border border-slate-700/30 p-5">
          <h3 className="text-sm font-semibold text-slate-200 mb-3">
            Sessoes de {new Date(selectedDate).toLocaleDateString("pt-PT", { weekday: "long", day: "numeric", month: "long" })}
          </h3>
          {eventosDoDia(selectedDate).length === 0 ? (
            ...
            <div className="space-y-2">
              {eventosDoDia(selectedDate).map((e) => (
                <div key={e.id} className="flex flex-wrap items-center justify-between gap-3 p-3 rounded-xl bg-slate-800/40 border border-slate-700/20">
                  <div>
                    <p className="text-sm font-medium text-slate-200">{e.acaoCodigo} – {e.acaoTitulo}</p>
                    <p className="text-xs text-slate-500">Sessao {e.numeroSessao} · {e.horaInicio}–{e.horaFim} · {e.modalidade} · [{e.estado}]</p>
                  </div>
                  <span className={`inline-flex items-center px-2 py-0.5 rounded text-[10px] font-medium ${
                    e.estado === "CONCLUIDA" ? "bg-green-500/10 text-green-400" :
                    e.estado === "EM_CURSO" ? "bg-blue-500/10 text-blue-400" :
