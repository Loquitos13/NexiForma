      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">
            <code className="text-purple-300 font-mono">{tenant?.slug}</code> · NIF {tenant?.nif} ·{" "}
            ...
            {tenant?.subscriptions[0]?.plan?.name ? (
              <span className="ml-2 inline-flex items-center px-2 py-0.5 rounded-md text-[11px] font-medium bg-teal-500/10 text-teal-300 border border-teal-500/20">
                Plano {tenant.subscriptions[0].plan.name}
              </span>
            ) : null}
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <Link
            ...
            className="rounded-lg bg-purple-600 px-3.5 py-2 text-sm font-medium text-white hover:bg-purple-500 transition-colors"
          >
            Centro de suporte do tenant
          </Link>
          <Link
            href={`/plataforma/tenantes/${id}/faturacao`}
            className="rounded-lg border border-purple-500/35 bg-purple-500/10 px-3.5 py-2 text-sm font-medium text-purple-200 hover:bg-purple-500/20 transition-colors"
          >
            Faturação AT
          </Link>
        </div>
      ...
      {tenant ? (
        <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3.5">
            ...
            <MiniStat label="Ações de Formação" value={tenant._count.acoesFormacao} />
            <MiniStat label="Formandos" value={tenant._count.formandos} />
            <MiniStat label="Sessões" value={tenant._count.sessoesFormacao} />
          </div>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-[1.05fr_1.35fr] gap-6 items-start">
            ...
            <div className="space-y-6">
              ...
              <div className="rounded-2xl bg-[#0c0a14]/80 border border-purple-500/10 p-5">
                ...
                <form onSubmit={(e) => void guardarDados(e)} className="grid gap-3">
                  ...
                  <div className="flex flex-wrap gap-2 pt-1">
                    ...
                    {tenant.status !== "ARCHIVED" ? (
                      <button type="button" disabled={editBusy} onClick={() => void arquivarTenant()}
                        className="px-3.5 py-2 rounded-lg border border-yellow-500/30 text-yellow-300 text-sm hover:bg-yellow-500/10 transition-colors cursor-pointer">
                        Arquivar
                      </button>
                    ) : null}
                    {tenant._count.users === 0 && tenant._count.acoesFormacao === 0 ? (
                      <button type="button" disabled={editBusy} onClick={() => void eliminarPermanente()}
                        className="px-3.5 py-2 rounded-lg border border-red-500/30 text-red-400 text-sm hover:bg-red-500/10 transition-colors cursor-pointer">
              ...
              <div className="rounded-2xl bg-[#0c0a14]/80 border border-yellow-500/15 p-5">
                ...
                <div className="space-y-3">
                  ...
                  <button type="button" disabled={impBusy || users.length === 0} onClick={() => void personificar()}
                    className="px-4 py-2 rounded-lg bg-yellow-600 hover:bg-yellow-500 disabled:opacity-50 text-white text-sm font-semibold transition-colors cursor-pointer">
              ...
              <div className="rounded-2xl bg-[#0c0a14]/80 border border-purple-500/10 p-5">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <button type="button" onClick={() => void gerarChave()}
                    className="px-3.5 py-1.5 rounded-lg bg-purple-600 hover:bg-purple-500 text-white text-sm font-medium transition-colors cursor-pointer">
                    Gerar chave
                  </button>
                </div>
            ...
            <div className="space-y-6">
              ...
              <div className="rounded-2xl bg-[#0c0a14]/80 border border-blue-500/15 p-5">
                  ...
                  <div className="mb-5 p-4 rounded-xl bg-emerald-950/40 border border-emerald-500/30 text-emerald-200 space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        ...
                        className="inline-flex items-center gap-1 text-xs px-2.5 py-1 rounded-md bg-emerald-600/30 hover:bg-emerald-600/50 border border-emerald-500/40 text-emerald-100 transition-colors cursor-pointer"
                      >
                        {copiedTempPassword ? (
                          <>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 text-xs font-mono">
                      <div className="p-2 rounded bg-black/40 border border-emerald-500/20">
                        ...
                        <span className="text-emerald-100 font-bold">{createdCredentials.slug}</span>
                      </div>
                      <div className="p-2 rounded bg-black/40 border border-emerald-500/20">
                ...
                <form onSubmit={(e) => void guardarGestor(e)} className="grid gap-3">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <label className="grid gap-1 text-xs text-slate-500">
                      Email do gestor *
                      <input
                  ...
                  <label className="grid gap-1 text-xs text-slate-500">
                    Palavra-passe temporária (opcional – gerada automaticamente se vazio)
              ...
              <div className="rounded-2xl bg-[#0c0a14]/80 border border-amber-500/15 p-5">
                ...
                <p className="text-xs text-slate-500 mb-4 leading-relaxed">
                  Bloqueio temporário após tentativas falhadas no login deste tenant. Valores globais:{" "}
                  ...
                  {lockoutHasCustom ? (
                    ...
                    <span className="block mt-1 text-slate-600">A usar valores globais (sem override).</span>
                  )}
                </p>
                <form onSubmit={(e) => void guardarLockout(e)} className="grid gap-3">
                ...
                <form onSubmit={(e) => void desbloquearUtilizador(e)} className="mt-5 pt-5 border-t border-amber-500/10 space-y-2">
                  ...
                  <div className="flex gap-2">
                    ...
                    />
                    <button type="submit" disabled={lockoutBusy} className={`${btnSecondaryClass} shrink-0`}>
                      Desbloquear
                    </button>
                  </div>
                </form>
              ...
              {/* Salas reais – Microsoft Teams & Zoom */}
              <div className="rounded-2xl bg-[#0c0a14]/80 border border-emerald-500/15 p-5">
                ...
                {/* Teams */}
                <div className="mb-5">
                  <h3 className="text-sm font-medium text-slate-300 mb-2">Microsoft 365 (por tenant cliente)</h3>
                ...
                {/* Zoom */}
                <div>
                  <h3 className="text-sm font-medium text-slate-300 mb-2">Zoom (conta do cliente)</h3>
                  <div className="space-y-2.5">
            ...
            {/* Utilizadores do Tenant em Largura Total */}
            <div className="col-span-1 lg:col-span-2 rounded-2xl bg-[#0c0a14]/80 border border-purple-500/20 p-5 shadow-xl">
              ...
              <div className="w-full max-w-lg rounded-2xl border border-emerald-500/40 bg-[#0c0a14] p-6 shadow-2xl text-slate-200 ring-1 ring-emerald-500/30">
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-slate-800">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        className="inline-flex items-center gap-1 rounded bg-emerald-600/30 hover:bg-emerald-600/50 border border-emerald-500/40 text-emerald-200 text-[11px] px-2 py-0.5 transition-colors cursor-pointer"
                      >
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 pt-2">
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 text-xs text-purple-300 hover:text-purple-100 transition-colors cursor-pointer"
                  >
